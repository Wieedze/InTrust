...
/**
 * @title TrustStakingFixed - FIXED VERSION
 * @dev Users stake native TRUST tokens and earn INTUIT rewards
 */
contract TrustStakingFixed is ReentrancyGuard, Ownable {
    ...
    struct UserStake {
        ...
        uint256 rewardsClaimed; // Track total rewards claimed
    }
    ...
    function unstake(uint256 amount) external nonReentrant {
        ...
        // FIXED: Reset lastStakeTime if fully unstaked
        if (userStake.amount == 0) {
    ...
    // FIXED: Calculate pending rewards for a user
    function calculatePendingRewards(address user) public view returns (uint256) {
        UserStake memory userStake = userStakes[user];
        if (userStake.amount == 0 || userStake.lastStakeTime == 0) return 0;
